...
//  public static final int PARAM_PLAYER_CALLBACK   = 0x1000 + 9;
    ...
    public NativePlayer(String url, Handler h, String params, String deviceName) {
        mPlayerMsgHandler = h;
        open(url, params, deviceName);
    }
    ...
    public long open(String url, String params, String deviceName) {
        if (m_hPlayer != 0L) {
            nativeClose(m_hPlayer);
        }
        m_hPlayer = nativeOpen(url, null, 0, 0, params, deviceName);
        return m_hPlayer;
    }
    ...
    public boolean initVideoSize(int rw, int rh, View v) {
        int vw = (int)getParam(NativePlayer.PARAM_VIDEO_WIDTH ); // video width
        int vh = (int)getParam(NativePlayer.PARAM_VIDEO_HEIGHT); // video height
    ...
    private native long nativeOpen (String url, Object surface, int w, int h, String params, String deviceName);
